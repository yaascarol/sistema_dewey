...
$total_livros_query = $conexao->query("SELECT COUNT(*) as total FROM livros");
$total_livros = 0;
if ($total_livros_query && $row = $total_livros_query->fetch_assoc()) {
    $total_livros = $row['total'];
}

$total_paginas = ceil($total_livros / $limite_por_pagina);
...
// **CONSULTA SQL CORRIGIDA AQUI**
$sql = "SELECT
            l.id,
            l.titulo,
            l.estoque,
            l.resumo,
            i.caminho AS capa, -- Coluna 'caminho' da tabela 'imagens' renomeada para 'capa'
            g.categoria AS genero_nome -- Coluna 'categoria' da tabela 'generos' renomeada para 'genero_nome'
        FROM
            livros l
        JOIN
            generos g ON l.genero_id = g.id
        LEFT JOIN -- Adicionado JOIN com a tabela 'imagens'
            imagens i ON l.id = i.livro_id
        ORDER BY
            l.titulo ASC
        ...
        <div class="paginacao">
            <?php if ($total_paginas > 1): ?>
                <?php if ($pagina_atual > 1): ?>
                    <a href="listar_livros.php?pagina=<?php echo $pagina_atual - 1; ?>" class="botao-paginacao">Anterior</a>
                <?php endif; ?>

                <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                    <a href="listar_livros.php?pagina=<?php echo $i; ?>"
                ...
                <?php if ($pagina_atual < $total_paginas): ?>
                    <a href="listar_livros.php?pagina=<?php echo $pagina_atual + 1; ?>" class="botao-paginacao">Próximo</a>
                <?php endif; ?>
            <?php endif; ?>
        </div>
    </div>
